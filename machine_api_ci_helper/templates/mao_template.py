...
<br>
<h2>Nodes</h2>
Node objects for your convenience
<br>
...
<div class="collapse" id="nodes">
    ...
    <button class="btn btn-primary k8sobjstatus{{ item.status }}" type="button" data-toggle="collapse" data-target="#node-{{ item.name }}" aria-expanded="false" aria-controls="node-{{ item.name }}">{{ item.name }}</button>
